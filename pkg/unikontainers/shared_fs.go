...
// TODO: Find and seet the correct size for the tmpfs in the host
...
func (s sharedfsRootfs) postSetup() error {
	...
	if err != nil {
		return fmt.Errorf("fauled to mount contianer's rootfs in monitor rootfs; %w", err)
	}

	newCntrRootfs := filepath.Join(s.monRootfs, containerRootfsMountPath)
	err = mountVolumes(newCntrRootfs, s.mounts)
	if err != nil {
		return fmt.Errorf("fauled to mount volumes in container's rootfs; %w", err)
	}
